...
class AbstractElement(et.Element):
    ...
    def to_html_string(self, indent: bool = True) -> str:
        ...
        if indent is True:
            et.indent(self)
        return et.tostring(
            self, encoding="unicode", method="html", short_empty_elements=True
        )
...
class SvgElement(AbstractElement):
    ...
    line = functools.partialmethod(basicshape, "line")
    circle = functools.partialmethod(basicshape, "circle")


class FactoryElement:

    converter = MdConverter(
        extensions=["pymdownx.extra", "pymdownx.keys", "pymdownx.mark"],
        output_format="html",
    )
    ...
    def from_markdown(
        self, content: [str | os.PathLike], parent: str = None, attr={}, **extra
    ):
        """
        Add element from a Markdown File or string written in markdown syntax
        you can choose, with 'parent' attribute the tag root element.
        ...
        try:
            ...
            print(f"{content} : Not a valid file, maybe a directory, try again")
            return
